...
ресурсы заранее, чтобы обеспечить пользователям как можно более быстрый доступ к сайту без необходимости дожидаться загрузки ресурсов.

Когда речь идёт о современных сайтах, оптимизация их скорости не ограничивается
только минимизацией исходного объёма загрузки. Работу сайтов можно ускорить,
...
понадобится, чтобы пользователям не приходилось ждать. Вы можете выбрать, какой контент подлежит
предварительной загрузке, путем анализа поведения ваших пользователей и
...
заголовков, управляющих кэшем, или они добавляются в кэш на
...
Если прогнозирующий алгоритм браузера достаточно уверен в своем
...
Chrome изучает топологию сети и ваши поведенческие паттерны при просмотре страниц.
Если ему удаётся справиться со своей работой, он может избавить пользователя от
сотен миллисекунд задержек при переходе между страницами и приблизить к святому
граалю «мгновенной загрузки страницы».

**Итак, если браузер делает за нас всё это, что ещё можно сделать, чтобы
улучшить его предугадывание и повысить производительность наших сайтов?**
...
отправке запроса на файл. На мобильных устройствах всё ещё дольше частично
из-за большей длительности этого цикла, которая составляет 200-1000 мс.
...
Как мы видели выше, в некоторых случаях Chrome делает это автоматически, но
даже тогда мы можем помочь ему, сделав уточнения конкретно для нашего сайта.
...
файл может быть быстро получен из кэша браузера.

Это значит, что когда мы уверены, что пользователь предпримет определённое
действие, мы можем начать загрузку важных ресурсов заранее, чтобы помочь
...
поддержки для сканера предварительной загрузки Chrome**. Значение `prefetch`
говорит браузеру, что нужно начать загружать файл `secondary.js`
когда он закончит со всеми критическими подресурсами текущей страницы,
подсказки с `prefetch` имеют минимальный приоритет.

Оптимальное время инициации предзагрузки зависит от оговорённого алгоритма
...
Предзагружаться должны только те элементы, которые могут быть закэшированы.
...
загрузить и отрисовать целую страницу, однако не показывать её
...
Вы можете вставлять подсказки по предварительной загрузке,
...
На данный момент способ проверить поддержку предзагрузки или предварительной
...
это можно проверить на странице **`chrome://net-internals/#prerender`**.
...
элементов и то, что DNS-предзагрузка будет называться «предварительное
